...
pv_jack_init (struct pv_complex * pv)
{
   ...
   client_name = (char *)malloc (sizeof (char) * 8);
...
void pv_complex_curses_jack (const char * file, long len, long hop_syn)
{
   int flag_window = 3;
   struct pv_complex * pv = NULL;
   SNDFILE * sf = NULL;
   SF_INFO sfinfo;
   struct pv_jack * pv_jack;
   int jack_sr;
   ...
   sf = sf_open (file, SFM_READ, &sfinfo); /* open input file */
   if (sf == NULL)
   {
   ...
   */
   ...
   /* jack initialization */
   pv_jack = pv_jack_init (pv);
   jack_sr = (int)jack_get_sample_rate (pv_jack->client);
